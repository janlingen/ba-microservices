...
import de.janlingen.inventarservice.saga.StringParser;
...
public class InventarService {
  ...
  public InventarService(InventarRepository inventarRepository, StringParser stringParser,
  ...
  @KafkaListener(topics = "bestellungPlaziert")
  public void bestellungBearbeiten(String bestellung){
    List<InventarPositionDTO> inventarPositionen = stringParser.parseInventarPositionenFromString(bestellung);
    LocalDateTime modifiziert = stringParser.parseModifiziertFromString(bestellung);
    if(checkVerfuegbarkeit(inventarPositionen) && LocalDateTime.now().minusSeconds(45).isBefore(modifiziert)){
      updatePositionen(inventarPositionen);
      kafkaTemplate.send("inventarErfolg", bestellung);
    } else {
      kafkaTemplate.send("inventarFehler", bestellung);
    }
  }

  @KafkaListener(topics = "inventarKorrigieren")
  public void inventarKorrigieren(String bestellung){
    List<InventarPositionDTO> inventarPositionen = stringParser.parseInventarPositionenFromString(bestellung);
    revertUpdatePositionen(inventarPositionen);
  }
  ...
  public void revertUpdatePositionen(List<InventarPositionDTO> inventarPositionDTOList) {
    for (InventarPositionDTO dto : inventarPositionDTOList) {
      InventarPosition inventarPosition = inventarRepository.findByName(dto.getName()).orElse(null);
      if (inventarPosition != null && dto.getAnzahl() >= 0) {
        Long currentAnzahl = inventarPosition.getAnzahl();
